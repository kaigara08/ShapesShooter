...
public class Enemy3Task : MonoBehaviour
{
    ...
    void Shot()
    {
        if (deletTime > shotSpan)
        {
            ...
            Vector3 beceVector = controller.ShotSpawnPos.forward;
            GeneratarShot(beceVector);
            GeneratarShot(Quaternion.Euler(0, 30, 0) * beceVector);
            GeneratarShot(Quaternion.Euler(0, -30, 0) * beceVector);
            deletTime = 0;
        }
